...
namespace BuildInspect.Controllers
{
    ...
    public class ERPController : SuperBaseController
    {
        ...
        public ActionResult SchoolCreate(SchoolMasterViewModel school)
        {
            if (ModelState.IsValid)
            {
                
                if (erpService.CheckSchool(school.School_Name, school.Zone) == false)
                {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult SchoolEdit(SchoolMasterViewModel school)
        {
            if (ModelState.IsValid)
            {
        ...
        public ActionResult ChecklistIndex()
        {
            try
            {
                ...
                if (grpid == 2)
                {
                    ...
                    var prjs = erpService.GetAllProjectsBasedSchoolIDs(schoolids, lastdate).Where(a => a.Month_Start_Date >= premonth_firstdate).ToList();
        ...
        public ActionResult SubmitChecklist(int PrjMasID, int SchoolID, string Frequency)
        {
            ...
            ViewBag.SchoolName = school.School_Name;
            ...
            if (user.ServiceType == "SP-LIFTSYS")
            {
                ViewBag.LIFT_count = erpService.GetLiftCount(SchoolID, usrid);
            }
        ...
        public ActionResult SubmitCheckList(CheckListTransMasterViewModel inputCLTM)
        {
           ...
           var result = erpService.SubmitChecklist(inputCLTM, inputCLTM.clTrnDetail, Path.Combine(Server.MapPath("~/images/CheckListFiles/" + AppSession.GetCurrentUserId() + "/")), halfPath);
            ...
            if (result > 0)
            {
                var schoolname = erpService.GetSchool(inputCLTM.SchoolID ?? default(int)).School_Name;
                if (user.ServiceType == "Electrical" || user.ServiceType == "Mechanical")
                {
        ...
        public ActionResult EditChecklist(int id)
        {
            ...
            {
                ViewBag.LIFT_count = erpService.GetLiftCount(clm.SchoolID ?? default(int), clm.UserID ?? default(int));
            }
            //ViewBag.LIFT_count = school.LIFT_count;
            ViewBag.HVLSFan_count = school.HVLSFan_count;
            ViewBag.CHILLER_count = school.CHILLER_count;
        ...
        public ActionResult PrintCheckList(int id)
        {
            ...
            else
            {
                if (clm.CLType == "Electrical")
                {
                    chklst = erpService.GetAllCheckListItems().Where(a => a.FormName.StartsWith("Electrical") && (a.Frequency == clm.Frequency || a.Frequency == null) && clArr.Contains(a.ServiceType)).ToList();
                }
                else if (clm.CLType == "Mechanical")
                {
                    chklst = erpService.GetAllCheckListItems().Where(a => a.FormName.StartsWith("Mechanical") && (a.Frequency == clm.Frequency || a.Frequency == null) && clArr.Contains(a.ServiceType)).ToList();
                }
                else
                {
                    chklst = erpService.GetAllCheckListItems().Where(a => a.FormName == formName && (a.Frequency == clm.Frequency || a.Frequency == null)).ToList();
                }
        ...
        public int GeneratePDF(int id, int mailflag)
        {
            try
            {
                ...
                if (mailflag == 1 || mailflag == 2)
                {
                    ...
                    {
                        sub = sub + "MKV - Maintenance Inspection for the School: " + school.School_Name + "-" + pmon + " " + styp;
                    ...
                    {
                        sub = sub + "HEC - Maintenance Inspection for the School: " + school.School_Name + "-" + pmon + " " + styp;
                    ...
                    if (school.CompanyID == 3)
                    {
                        sub = sub + "PPL - Maintenance Inspection for the School: " + school.School_Name + "-" + pmon + " " + styp;
        ...
        public ActionResult PrintCheckListPdf(int id, string cltype)
        {
            ...
            else
            {
                if (clm.CLType == "Electrical")
                {
                    chklst = erpService.GetAllCheckListItems().Where(a => a.FormName.StartsWith("Electrical") && (a.Frequency == clm.Frequency || a.Frequency == null) && a.ServiceType == cltype).ToList();
                }
                else if (clm.CLType == "Mechanical")
                {
                    chklst = erpService.GetAllCheckListItems().Where(a => a.FormName.StartsWith("Mechanical") && (a.Frequency == clm.Frequency || a.Frequency == null) && a.ServiceType == cltype).ToList();
                }
                else
                {
                    chklst = erpService.GetAllCheckListItems().Where(a => a.FormName == formName && (a.Frequency == clm.Frequency || a.Frequency == null) && a.ServiceType == cltype).ToList();
                }
        ...
        public ActionResult ServiceFormIndex()
        {
            ...
            {
                var obj = school.Where(a => a.SchoolID == sfitem.SchoolID).SingleOrDefault();
                ...
                sfitem.SchoolName = obj.School_Name;
        ...
        public ActionResult ServiceFormSave(int id)
        {
            ...
            svform.SchoolName = school.School_Name;
        ...
        {                  
            var chklst = erpService.GetAllCheckListItems().Where(a => a.ServiceType == CLType && (a.Frequency == Frequency || a.Frequency == null)).ToList();
        ...
        public ActionResult CLDownload()
        {
            ...
            var usrid = AppSession.GetCurrentUserId();
            var user = userService.GetUser(usrid);
            var school = new List<SchoolMasterViewModel>();
            if (user.Zone != "ALL")
            {
        ...
        public ActionResult CLDownload(FilterViewModel filter)
        {
            if (ModelState.IsValid)
            {
                ...
                var lst = erpService.GetFilterCLDownload(filter);
                fil.clfdownload = lst;
        ...
        public FileResult DownloadZipFile(int schoolid, string month, string year)
        {
           ...
           // {
                ...
                var schname = school.Zone + "_" + school.School_Name + "_" + freq;
